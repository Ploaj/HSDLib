...
namespace HSDRawViewer.GUI.Plugins.GrTool
{
    ...
    public class GrRenderResource : IDisposable
    {
        ...
        public bool DrawAnimationArc { get; set; } = true;


        private List<Vector3> AnimationArc = new List<Vector3>();
        ...
        public void LoadRailAnimation(HSD_AnimJoint j)
        {
            ...
            // sample animation arc
            if (_animManager.Nodes.Count > 0)
            {
                RailJoint.ResetTransforms();
                for (int i = 0; i < FrameCount; i++)
                {
                    RailJoint.ApplyAnimation(_animManager.Nodes[0].Tracks, i);
                    RailJoint.RecalculateTransforms(Camera, true);
                    AnimationArc.Add(RailJoint.WorldTransform.ExtractTranslation());
                }
            }
