...
namespace HSDLib.Helpers.TriangleConverter
{
    public class TriangleConverter
    {
        ...
        public List<PrimitiveGroup> GroupPrimitives(GXVertex[] points, out int pointCount, out int faceCount)
        {
            pointCount = 0;
            faceCount = 0;
            ...
            if (_useStrips)
            {
                ...
                remapData.Remap<GXVertex>(points, null);
                ...
                TriStripper stripper = new TriStripper(
                    ...
                    points.Select(x => x.PMXID).ToArray(),
                    remapData._impTable);
