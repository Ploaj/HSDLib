...
namespace HSDLib.Helpers
{
    ...
    public class TPL : IDisposable
    {
        ...
        public static byte[] ConvertToTextureMelee(Image image, int format, int palformat, out byte[] paletteData)
        {
            TPL tmpTpl;
            paletteData = null;

            if (format == (int)TPL_TextureFormat.CI14X2 ||
                format == (int)TPL_TextureFormat.CI4 ||
                format == (int)TPL_TextureFormat.CI8)
            {
                tmpTpl = FromImage(image, (TPL_TextureFormat)format, (TPL_PaletteFormat)palformat);
                paletteData = tmpTpl.paletteData[0];
            }
            else
            {
                tmpTpl = FromImage(image, (TPL_TextureFormat)format);
            }
        ...
        public static int PaletteByteSize(TPL_PaletteFormat Format, int ColorCount)
        {
            switch (Format)
            {
        ...
        public static int TextureByteSize(TPL_TextureFormat tplFormat, int width, int height)
        {
            switch (tplFormat)
            {
                ...
                case TPL_TextureFormat.CMP:
                    return Shared.AddPadding(width, 8) * Shared.AddPadding(height, 8) / 2;
                default:
                    throw new FormatException("Unsupported Texture Format! " + tplFormat);
            }
        }
